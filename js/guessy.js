(function (){
    ...
    function init() {
        ...
        correctEmoticon = hexagons__check[solutionIndex];
        ...
        document.querySelector('.game__hint--icon').innerHTML = hexagons[solutionIndex].children[1].innerHTML;
    }
    ...
    for (var i = 0; i < hexagons__check.length; ++i) {
        hexagons__check[i].addEventListener('change', function(){
            if (this.checked) {
                ...
                if (this.id.replace('hexagon--', '') === String(solutionIndex + 1)) {
                    document.querySelector('.game__hint--story').innerHTML = "'Woaw you have done it' <br> click ``Find me`` to restart.";
                }
